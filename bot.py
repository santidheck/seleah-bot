...
ALLOWED_USERS  = [
    int(x) for x in os.environ.get("ALLOWED_USER_IDS", "").split(",") if x
]
...
async def handle_voice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "No pude procesar el audio. Intentalo de nuevo o escribelo."
        )
    finally:
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log.error(str(e))
        await update.message.reply_text(
            "No entendi. Intenta: 'botox a Maria, 3500 en efectivo'"
        )
...
async def process_extracted(update: Update, ctx: ContextTypes.DEFAULT_TYPE, data: dict):
    ...
    if tipo == "venta":
        ...
        await update.message.reply_text(
            "No identifique si es venta, gasto o consulta. Se mas especifica."
        )
        return
