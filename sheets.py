...
VENTAS_COLS   = [“ID”,“Fecha”,“Cliente”,“Servicio_ID”,“Servicio_Nombre”,
“Precio”,“Costo”,“Margen”,“Metodo_Pago”,“Pagado”,“Notas”,“Fuente”]
GASTOS_COLS   = [“ID”,“Fecha”,“Categoria”,“Descripcion”,“Monto”,“Fuente”]
CLIENTES_COLS = [“ID”,“Nombre”,“Telefono”,“Email”,“Primera_Visita”,“Visitas”,“Total_Historico”]
...
def _get_or_create(self, name: str):
    try:
        ...
        ws = self.sh.add_worksheet(title=name, rows=1000, cols=20)
        return ws
...
def _update_client(self, name: Optional[str], amount: float):
    if not name or name == "Sin nombre": return
    ws = self.sh.worksheet("Clientes")
    records = ws.get_all_records()
    existing = next((r for r in records if r["Nombre"].lower() == name.lower()), None)
    if existing:
        # find row and update
        row_idx = records.index(existing) + 2  # +2 for header
        ws.update_cell(row_idx, 6, int(existing["Visitas"]) + 1)
        ws.update_cell(row_idx, 7, float(existing["Total_Historico"]) + amount)
    else:
        ws.append_row([
...
def get_monthly_summary(self, year_month: str = None) -> dict:
    ...
    mes_ventas  = [v for v in ventas  if str(v.get("Fecha","")).startswith(ym)]
    mes_gastos  = [g for g in gastos  if str(g.get("Fecha","")).startswith(ym)]
    ...
    return {
        ...
        "pendientes":  len([v for v in mes_ventas if v.get("Pagado") == "Pendiente"]),
    }
...
def get_top_services(self, year_month: str = None) -> list:
    ...
    for v in mes:
        ...
        if sid not in svc_map:
            svc_map[sid] = {"nombre": sid, "sesiones": 0, "ingreso": 0, "margen": 0}
        svc_map[sid]["sesiones"] += 1
        svc_map[sid]["ingreso"]  += float(v.get("Precio", 0))
        svc_map[sid]["margen"]   += float(v.get("Margen", 0))
    
    return sorted(svc_map.values(), key=lambda x: x["ingreso"], reverse=True)
